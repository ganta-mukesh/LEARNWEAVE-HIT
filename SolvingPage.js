...
const languageToCompilerMap = {
  ...
  SQL: "sql",
  C: "c",
  CPP: "cpp",        // Added C++ support
  JAVA: "java",      // Added Java support
  MONGODB: "mongodb",
  "AI/ML": "python",
};
...
const SolvingPage = () => {
  ...
  const { language, challenge: initialChallenge, challengeType } = location.state || {};
  ...
  const [challengeTypeFilter, setChallengeTypeFilter] = useState(challengeType || "all");
  ...
  useEffect(() => {
    if (initialChallenge) {
        ...
        setCompilerLanguage(languageToCompilerMap[initialChallenge.language] || "javascript");
    }
  }, [initialChallenge]);
  ...
  // Show a message when coming from placement page
  const isFromPlacementPage = challengeType && challengeType !== "all";
    ...
    <div className="slv-solving-page-container">
          ...
          <>
            {isFromPlacementPage && (
              <div className="placement-notice">
                <h3>📝 Placement Preparation Questions</h3>
                <p>These questions are specifically curated for placement preparation.</p>
              </div>
            )}
                ...
                filteredQuestions.map((question) => (
                  <div key={question._id} className="slv-question-card">
                    ...
                    <div className="question-meta">
                      ...
                      {question.type && <span className="type-badge">{question.type}</span>}
                    </div>
                    <button
      ...
      <div className="slv-compiler-section">
        <div className="compiler-tabs">
          ...
          >
            {Object.entries(languageToCompilerMap).map(([lang, compLang]) => (
              <option key={lang} value={compLang}>
                {lang}
              </option>
            ))}
          </select>
          <button
            ...
            onClick={handleRun}
            disabled={loading || isChallengeSolved}
          >
            {loading ? (
          ...
          <button
            className={`help-button ${helpLoading ? 'loading' : ''}`}
            onClick={handleHelp}
